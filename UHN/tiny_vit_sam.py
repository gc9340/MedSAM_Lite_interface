...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        # Improve runtime using pytorch sdpa
        sdpa = F.scaled_dot_product_attention(q, k, v, attn_mask = self.ab, scale = self.scale)
        x = sdpa.transpose(1,2).reshape(B, N, self.dh)
        x = self.proj(x)
        return x
